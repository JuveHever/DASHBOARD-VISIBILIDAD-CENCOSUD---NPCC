...
function Dashboard({ data, revisado, error, recargar }) {
  ...
  const tipoStats = useMemo(() => {
    ...
    pdvs.forEach(p => p.tipos.forEach(t => {
      const e = m.get(t.tipo) || { tipo: t.tipo, pdv: 0, fotos: 0 }
      e.pdv += 1; e.fotos += t.nFotos
      m.set(t.tipo, e)
    }))
  ...
  const abrir = useCallback((p, i) => {
    const fotos = p.tipos.flatMap(t => t.fotos.map(f => ({ ...f, tipo: t.tipo })))
    ...
    <>
      ...
      <main className="wrap">
        ...
        <section className="rail-sec">
          ...
          <div className="reads">
            ...
            <div className="read">
              ...
              <div className="l">Fotos disponibles</div>
            </div>
          </div>
        </section>
        ...
        <section className="cards">
          {filtrados.map(p => {
            const fotos = p.tipos.flatMap(t => t.fotos.map(f => ({ ...f, tipo: t.tipo })))
            const vista = fotos.slice(0, 4)
            const resto = fotos.length - vista.length
            return (
              <article className="card" key={p.codigo}>
                ...
                {p.tipos.length > 0 && (
                  <div className="chips">
                    {p.tipos.map(t => (
                      <span className="chip" key={t.tipo}>
                        <i className="dot" style={{ background: color(t.tipo) }} />
                        {t.tipo}
                        {t.nFotos > 1 && <span className="k">{t.nFotos}</span>}
                      </span>
                    ))}
                  </div>
                )}
                ...
                {vista.length > 0 ? (
                  <div className="thumbs">
                    {vista.map((f, i) => (
                      <button className="thumb" key={f.cod + i} onClick={() => abrir(p, i)}
                        title={`${f.tipo} · ${f.fecha}`}>
                        <img src={f.url} alt={`${f.tipo} en ${p.nombre}`} loading="lazy" />
                        <i className="tag" style={{ background: color(f.tipo) }} />
                      </button>
                    ))}
                    {resto > 0 && (
                      <button className="thumb more" onClick={() => abrir(p, 4)}>+{resto}</button>
                    )}
                  </div>
                ) : (
      ...
      <div className="lb-in">
        ...
        <div className="lb-bar">
          <div>
            ...
            <div className="m">
              {pdv.nombre} · {f.fecha} · registrado como “{f.raw || 's/d'}”
            </div>
            <div className="m">
